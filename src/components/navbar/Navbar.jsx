  ...
  const renderNestedNav = (list = [], isRoot = false) => {
    return list.map((nav) => {
      if (isRoot) {
        ...
        //   );
        // }
        if (nav.link && !nav.sub_items.length) {
          return (
            <StyledNavLink href={nav.link} key={nav.title}>
              {nav.title}
            </StyledNavLink>
          );
        }
        if (nav.sub_items.length) {
          return (
            <FlexBox
              key={nav.title}
              alignItems="center"
        ...
        if (nav.link) {
          return (
            <NavLink href={nav.link} key={nav.title}>
              <MenuItem style={{ color: theme.palette.secondary[500] }}>
                {nav.title}
              </MenuItem>
            </NavLink>
          );
    ...
    <NavBarWrapper hoverEffect={false} elevation={elevation} border={border}>
      {!hideCategories ? (
        <InnerContainer>
          ...
          {/* Horizontal menu */}
          <FlexBox gap={4}>
            {renderNestedNav(siteSettingsData.main_menu, true)}
          </FlexBox>
        </InnerContainer>
      ) : (
        <InnerContainer
